...
`merge_scores` 를 `d == 오늘` 조건 **하나로만** 읽고(`runQuery` + `fieldFilter EQUAL`, limit 500) 정렬은 화면에서 합니다. 같음 조건 하나는 단일 필드 자동 인덱스로 되지만, 여기에 `orderBy s` 를 붙이면 복합 인덱스(d, s)를 콘솔에서 따로 만들어야 하고 그 전까지 400 이 납니다. 가족 규모에서는 화면 정렬이 충분합니다.

## Firestore 규칙 — 2026-09-13 콘솔에 게시 완료

> 아래 절은 게시 전 상황의 기록입니다. 지금은 `docs/firestore.rules` 가 콘솔에 올라가 있고, `merge_scores`·`merge_progress` 읽기가 200 으로 확인됐습니다.
